...
class Passengers extends Component {
    ...
    handleClick = (event, idx) => {
        ...
        if (!row.closest("form").reportValidity()) {
            return;
        }
        ...
        this.values.push({ "id": this.state.id++, "pname": pname, "gender": gender, "age": age })

        console.log("Values :" + JSON.stringify(this.values.length))
        this.setState({ info: true, lastAddedPname: pname })

        // Clear inputs and disable this specific button if you want, 
        // but here we just lock the whole thing when all added.
        ...
        for (var i = 0; i < this.state.npsgn; i++) {
            ...
            fieldsArray.push(
                <tr>
                    ...
                    <td>
                        <select class="btn btn-info" name='gender' required>
                    ...
                    <td align='center'>
                        <button disabled={this.state.btn} class="btn btn-warning " onClick={(e) => this.handleClick(e, i)}>Add Passenger</button>
                    </td>
                </tr>
            );
        }
                                    ...
                                    <div class="bg-white shadow-sm pt-4 pl-2 pr-2 pb-2">
                                        <div class="tab-content">
                                            {this.state.info && <div class="alert alert-success">
                                                <strong>Success!</strong>&nbsp; Passenger added : &nbsp; {this.state.lastAddedPname}
                                            </div>}
                                            ...
                                            <div class="tab-pane fade show active pt-3">
                                                <form>
                                                    ...
                                                    <div class="card-footer">
                                                        <button onClick={this.savePassenger} type="submit" disabled={!this.state.btn} class="subscribe btn btn-primary btn-block shadow-sm"> Book Ticket</button>
                                                    </div>
